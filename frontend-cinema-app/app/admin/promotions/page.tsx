...
export default function PromotionsPage() {
    ...
    async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        if (!promoCode || !startDate || !endDate || !(discountPercent > 0 && discountPercent <= 100)) {
            setErr("Please provide a code, valid dates, and a discount between 1 and 100.");
            setSubmitting(false);
            return;
        }
        ...
        try {
            const createRes = await fetch("/api/promotions", {
                ...
                body: JSON.stringify({ promoCode, startDate, endDate, discountPercent }),
            });
            if (!createRes.ok) throw new Error(`Create failed (${createRes.status})`);
            const created = await createRes.json();

            if (emailNow && created?.id) {
                const emailRes = await fetch(`/api/promotions/${encodeURIComponent(created.id)}/email`, { method: "POST" });
        ...
        <main className="relative z-10 mx-auto max-w-2xl px-4 py-8 space-y-6">
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <label className="inline-flex items-center gap-2">
                    <input type="checkbox" name="emailNow" />
                    <span>Email to subscribed users now</span>
                </label>
