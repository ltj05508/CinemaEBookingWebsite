...
export default function BookingPage({ params }: PageProps) {
  // Unwrap params (App Router delivers a promise here because of "use client")
  const { id, showtime: rawShowtime } = use(
    (params ?? Promise.resolve({ id: '', showtime: '' })) as Promise<{
      id: string;
      showtime: string;
    }>
  ) as { id: string; showtime: string };
  ...
  const proceedToCheckout = () => {
    ...
    const seatsParam = encodeURIComponent(selectedArray.join(','));
    const showtimeParam = encodeURIComponent(showtime);
    const showtimeIdParam = showroom.showtimeId
      ? `&showtimeId=${showroom.showtimeId}`
      : '';
    router.push(
      `/movies/${id}/checkout?showtime=${showtimeParam}&seats=${seatsParam}${showtimeIdParam}`
    );
  };
...
function Row({
  ...
  return (
    <>
      ...
      {Array.from({ length: cols }, (_, i) => {
        const seatId: SeatId = `${row}${i + 1}`; // fixed ID format (A1, C5, etc.)
